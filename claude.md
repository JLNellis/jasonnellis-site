...
- Tools index: `/tools` is generated by `tools.njk` from the `tools`
  ...
  `tool-index/tool-index.json`). The homepage `05 · Tools` teaser and
  ...
  `tool-index/`, not `tools/`, because `tools/` holds build scripts.
...
| `tools.njk` + `tool-index/*.md` | `/tools` | Tools index | Eleventy-generated — see "Writing archive (Eleventy)". Lists The Feed, Burn Rate, and The Lens as cards with a Substack banner. In `NAV_LINKS` as "Tools". |
| `lens.html` | `/lens` | The Lens — creator durability framework | Three questions ("tells") for reading whether a creator is building or renting attention. Linked from `/blog` hero, `/speaking`, `/advisory`, `/tools`. |
| `burn-rate.html` | `/burn-rate` | Burn Rate — creator cadence calculator | Standalone-styled but loads `nav.js`. Substack-gated "what to cut" plan (unlock flag in `localStorage`). Indexed since 2026-09-14; listed on `/tools`. OG image built from `tools/burn-rate-og/`. |
...
- Live at `/the-feed` and `thefeed.jasonnellis.com`; indexed and listed on
  `/tools` (via `tool-index/the-feed.md`) since 2026-09-14. Not a top-level
  nav item.
