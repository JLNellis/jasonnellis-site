...
| `advisory.html` | `/advisory` | Advisory / contract work | Reworked Sept 2026 around the "CEO whisperer" ladder (internal name only — never on the page): **The Second Chair** (3 months counsel next to the CEO, $18k, placeholder name) and **The Six-Month Sprint** (Second Chair + fractional CPO shipping one named goal, $72k fixed over six months, billed $12k monthly, + equity), plus a one-line "full time is a different conversation" note (deliberately not a card — the site is not job seeking) and the standalone workshop callout ($5k half day / $8k full day, plus travel if in person). Hero names the niche (media, creator and rights businesses); the stat band under the hero labels each number with its company inline and deliberately carries no SuperBam figure (the old $70k→$20k overhead stat was pulled Sept 2026: internal to SuperBam, and it reads as "he cuts" to a founder who's raising). Copy was de-AI'd the same month: at most one "not X, it's Y" construction on the page, no em-dash asides. The Read and The Sixth Week were retired; The Read's diagnostic became the free intake strip (`.intake`: 90-min call → read the reports → written assessment → signed agreement), which is the page's CTA ("Ask for the first call" — deliberately not "Book": there is no scheduler behind it, just the contact form). Who-it's-for, how-I-work terms, case-study links, CTA prefills `?topic=advisory`. Prices live in this file and in the `budget` dropdown in `contact.html` — change both together. Homepage offer card and the contact FAQ mirror the same ladder. |
